...
class AccountInvoice(models.Model):
    ...
    @api.model
    def _get_payment_type_domain(self):
        if self._context.get('default_type') in ('in_invoice', 'in_refund', 'in_receipt'):
    ...
    payment_mode_id = fields.Many2one(
        ...
        domain=lambda self: self._get_payment_type_domain())
    bank_account_required = fields.Boolean(
        related='payment_mode_id.payment_method_id.bank_account_required',
        readonly=True)
    invoice_partner_bank_id = fields.Many2one(ondelete='restrict')
        ...
        if self.partner_id:
            if self.type == 'in_invoice':
                ...
                if (
                    ...
                    self.invoice_partner_bank_id = False
    ...
    def line_get_convert(self, line, part):
        ...
        if line.get('type') == 'dest' and line.get('move_id'):
            invoice = self.browse(line['move_id'])
    ...
    @api.model
    def _prepare_refund(
            self, invoice, invoice_date=None, date=None, description=None,
            journal_id=None):
    ...
    @api.constrains('partner_id', 'invoice_partner_bank_id')
    ...
    def partner_banks_to_show(self):
        ...
        if self.payment_mode_id.payment_method_id.code == \
            ...
            return (self.mandate_id.invoice_partner_bank_id or
                    self.partner_id.valid_mandate_id.invoice_partner_bank_id)
